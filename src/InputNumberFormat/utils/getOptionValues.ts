...
export default function getOptionValues(
  ...
  const localeSeparator = new Intl.NumberFormat(locales).format(1.1)[1];

  // Получаем все цыфры в заданной локали (возможны варианты
  // с китайской десятичной системой и арабскими цифрами)
  let localeSymbols = new Intl.NumberFormat(locales, { useGrouping: false }).format(1234567890);

  localeSymbols = localeSymbols[9] + localeSymbols.slice(0, -1);
  ...
  const minimumFractionDigits = (numberFormatter.format(0).split(localeSeparator)[1] ?? '').length;

  const maximumFractionDigits = (
    numberFormatter.format(Number(`0.${'1'.repeat(30)}`)).split(localeSeparator)[1] ?? ''
  ).length;
